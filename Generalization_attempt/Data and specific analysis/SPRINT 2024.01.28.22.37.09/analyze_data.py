...
class SprintPlannedSequence(PlannedSequence):
    ...
    def calculate_result_vector(self, others=None, index=None):
        ...
        condition = 0
        if (last_detection_pulse_reflected_photons > 0) and (reflections >= 1) and (next_reflections >= 1):
        ...
        condition = 1
        if (last_detection_pulse_reflected_photons > 0) and \
...
# north_counts = \
#     np.histogram([val % SEQUENCE_LENGTH for arr in experiment_data[0] for val in arr],
#                  bins=np.arange(SEQUENCE_LENGTH + 1))[
#         0]
# south_counts = \
#     np.histogram([val % SEQUENCE_LENGTH for arr in experiment_data[1] for val in arr],
#                  bins=np.arange(SEQUENCE_LENGTH + 1))[
#         0]
#
# plt.plot(north_counts)
# plt.plot(south_counts)
# plt.show()
